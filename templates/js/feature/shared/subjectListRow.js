...
export function {{titlecase .Object.Name}}ListRow(props) {
	...
	return (
		<div className='flex flex-row justify-between py-2 items-center w-full'>
			...
			<RowThumbnail source={'https://storage.googleapis.com/{{.Config.ProjectName}}-uploads/'+props.item.Meta.Media.URIs[props.item.Meta.Media.URIs.length-1]}/>
			{{end}}
			...
			{{if eq "pay" .Object.Options.Handcash.Type }}
			<RowPay id={props.id} item={props.item}/>
			{{end}}
			{{if eq "mint" .Object.Options.Handcash.Type }}
			<RowMint id={props.id} item={props.item}/>
			{{end}}
