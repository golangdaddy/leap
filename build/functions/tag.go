...
// api-tag
func EntrypointTAG(w http.ResponseWriter, r *http.Request) {
	...
	tag := &models.TAG{}
	if err := utils.GetDocument(app, id, tag); err != nil {
	...
	case "GET":
		...
		// return a specific tag object by id
		case "object":

			cloudfunc.ServeJSON(w, tag)
			return
	...
	case "DELETE":
		...
		_, err := tag.Meta.Firestore(app).Delete(app.Context())
		if err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return
		}
...
func getTagList(app *common.App, subject *models.TAG) []*models.TAG {
	list := []*models.TAG{}
