...
func EntrypointATTRIBUTES(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "archiveupload":
			...
			for n, zipFile := range zipReader.File {
				...
				log.Println("creating new attribute:", zipFile.Name)
		
				fields := models.FieldsATTRIBUTE{}
				attribute := models.NewATTRIBUTE(parent, fields)

				attribute.Meta.Context.Order = n

				// generate a new URI
				uri := attribute.Meta.NewURI()
				println ("URI", uri)

				bucketName := "test-project-db-uploads"
				if err := writeAttributeFile(app, bucketName, uri, fileBytes); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
	...
	case "GET":
		...
		case "count":
			...
			data := map[string]int{
				"count": parent.FirestoreCount(app, "attributes"),
			}
		...
		case "list", "altlist":
			...
			q := parent.Firestore(app).Collection("attributes").OrderBy("Meta.Modified", firestore.Desc)
...
func writeAttributeFile(app *common.App, bucketName, objectName string, content []byte) error {
