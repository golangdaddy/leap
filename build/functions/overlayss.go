...
// api-overlayss
func EntrypointOVERLAYSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := models.FieldsOVERLAYS{}
			overlays := models.NewOVERLAYS(parent, fields)
			if !overlays.ValidateInput(w, m) {
				return
			}

			log.Println(*overlays)

			// write new OVERLAYS to the DB
			...
			// finish the request
			if err := cloudfunc.ServeJSON(w, overlays); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
			return
	...
	case "GET":
		...
		// return the total amount of overlayss
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app, "overlayss"),
			}
		...
		// return a list of overlayss in a specific parent
		case "list", "altlist":
			...
			list := []*models.OVERLAYS{}

			q := parent.Firestore(app).Collection("overlayss").OrderBy("Meta.Modified", firestore.Desc)
			if limit > 0 {
				q = q.Limit(limit)
			}
			...
			for {
				...
				if err := doc.DataTo(overlays); err != nil {
					log.Println(err)
					continue
				}
				list = append(list, overlays)
			}
