...
// api-overlay
func EntrypointOVERLAY(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.GetDocument(app, id, overlay); err != nil {
	...
	case "POST":
		...
		case "upload":
			...
			objectName := overlay.Meta.NewURI()
	...
	case "GET":
		...
		// return a specific overlay object by id
		case "object":

			cloudfunc.ServeJSON(w, overlay)
			return
	...
	case "DELETE":
		...
		_, err := overlay.Meta.Firestore(app).Delete(app.Context())
		if err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return
		}
...
func getOverlayList(app *common.App, subject *models.OVERLAY) []*models.OVERLAY {
	list := []*models.OVERLAY{}
