	...
	case "POST":
		...
		case "archiveupload":
			...
			for n, zipFile := range zipReader.File {
				...
				log.Println("creating new collection:", zipFile.Name)
		
				fields := models.FieldsCOLLECTION{}
				collection := models.NewCOLLECTION(parent, fields)

				collection.Meta.Context.Order = n

				// generate a new URI
				uri := collection.Meta.NewURI()
				println ("URI", uri)

				bucketName := "test-project-db-uploads"
				if err := writeCollectionFile(app, bucketName, uri, fileBytes); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
...
func writeCollectionFile(app *common.App, bucketName, objectName string, content []byte) error {
