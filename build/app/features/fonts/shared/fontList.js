...
import { FontDELETE, FontsListGET, FontMoveUpPOST, FontMoveDownPOST } from '../_fetch';

import VisitTab from '@/features/interfaces';

import Loading from '@/app/loading';
import { Preview } from './font';
...
export function FontList(props) {
	...
	function updateList() {
		FontsListGET(userdata, props.subject?.Meta.ID, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectItem(id) {
		console.log("SELECT Font", id)
		const next = list[parseInt(id)]
		const context = {
			"_": next.name,
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "font", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		FontMoveUpPOST(userdata, object.Meta.ID)
	...
	function moveDown(id) {
		...
		FontMoveDownPOST(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		FontDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
