...
func getEditInputs(object *models.Object, field *models.Field) (string, error) {
	...
	switch strings.ToLower(field.Element.Name) {
	case "select":
		const s = `<Select id="{{lowercase .Name}}" type='text' required={ {{.Required}} } reference={ "{{.InputReference}}" } referenceParent={ subject } title="%s {{lowercase .Name}}" options={ {{json .InputOptions}} } placeholder="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "string", "date", "week", "month", "time", "phone", "email", "color":
		const s = `<Input id="{{lowercase .Name}}" type='{{.Type}}' required={ {{.Required}} } title="%s {{lowercase .Name}}" placeholder="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "uint":
		const s = `<Input id="{{lowercase .Name}}" type='number' required={ {{.Required}} } title="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "int":
		const s = `<Input id="{{lowercase .Name}}" type='number' required={ {{.Required}} } title="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
	...
	case "float":
		...
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "text":
		const s = `<Textarea id="{{lowercase .Name}}" required={ {{.Required}} } title="%s {{lowercase .Name}}" placeholder="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "checkbox":
		const s = `<Checkbox id="{{lowercase .Name}}" required={ {{.Required}} } title="%s {{lowercase .Name}}" placeholder="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "object":
		const s = `<Object id="{{lowercase .Name}}" required={ {{.Required}} } title="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
		output = fmt.Sprintf(s, object.Name)
		tmp, err = template.New(object.Name + "_" + field.Name).Funcs(funcMap).Parse(output)
	case "arraystring":
		const s = `<Object id="{{lowercase .Name}}" required={ {{.Required}} } title="%s {{lowercase .Name}}" inputChange={handleInputChange} value={ inputs["{{lowercase .ID}}"].value } />`
